...
describe("Task 7 final consistency contract", () => {
  ...
  it("keeps active gameplay on one explicit decomposed playfield architecture", () => {
    ...
    expect(arcadePlayfieldArchitecture).not.toContain("5d-selector-level-two-detailed-stage_89e2157b.webp");
    expect(arcadePlayfieldArchitecture).not.toContain("selectah-splash-art-direction_4d1c250f.webp");
    expect(arcadePlayfieldArchitecture).not.toContain("5d-selector-rave-stage_e4fdff4b.webp");
    expect(gameSource).toContain("level-two-club-layers");
    expect(gameSource).toContain("level2-club-backwall_0fefe2b1.webp");
    expect(gameSource).toContain("level2-club-architecture_b59eb6c5.webp");
    expect(gameSource).toContain("level2-club-speaker-edges_2cd0f111.webp");
    expect(gameSource).toContain("level2-club-crowd-bands_f8a90d30.webp");
    expect(gameSource).toContain("level2-club-reactive-fx_59e2c1c8.webp");
  ...
  it("triggers Crowd Pressure once from 15 clean real Level 1 dubplates and returns to Level 1", () => {
    ...
    expect(gameSource).not.toContain('className="equipment-condition-callout"');
    expect(gameSource).toContain('aria-label={`Mixer recovery progress: ${recoveryProgress} of 3`}');
  ...
  it("keeps Level 3 Pit Run as an approved-art three-frame run with compact feedback", () => {
    ...
    expect(gameSource).toContain('/assets/approved-pit-run-afterparty-board_b135c77c.png');
