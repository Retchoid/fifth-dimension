...
const requirements: Array<[string, string, string]> = [
  ...
  ["durable bonus-clear crown proof", game, "completedLevel === \"level2\" && bonusCompletedRef.current"],
  ...
  ["two-cop seizure threshold", game, "if (policeBadgeHitsRef.current >= 2)"],
  ["same-type bottle/core crowd threshold", game, "if (hazardHitsRef.current >= 2)"],
  ["three-pill overload threshold", game, "if (pillHitsRef.current >= 3)"],
