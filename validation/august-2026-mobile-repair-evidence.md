...
After the deployment propagation completed, the public stylesheet contained the detailed splash-direction artwork, speaker-stack artwork, and turntable artwork references. This confirms the composed illustrated-scene layer is present on the live bundle rather than only the development build.

The subsequent cascade correction also propagated to production. The public bundle now contains the high-specificity `.game-viewport .loss-curb-overlay` rule along with the dedicated loss, game-over, and police illustrated asset references. The previous generic overlay background is therefore no longer the winning production rule for those detailed scene states.

Direct development-state inspection confirmed that the mounted `.police-seizure-overlay` computes to the composed `selectah-splash-art-direction` image with layered dark-red scene shading and `cover` sizing. This verifies that the running game, rather than only the compiled stylesheet, now renders an illustrated police-scene background.

Fresh deterministic state checks also verified the live loss and game-over sequence. The mounted loss-comedown overlay resolves to the `selectah-urban-brawler-visual-target` artwork with `cover` sizing. After the comedown completed, the restored game-over screen mounted with the heading **SESSION TERMINATED** and the dedicated `5d-selector-rave-stage` background, also at `cover` sizing. The detailed scene layer therefore remains active through the full negative end-state path.

The published checkpoint now exposes the `#selectah-showdown` anchor. A direct public load landed at the arcade section with the target present and positioned at the top of the viewport, confirming that the added deep link does not disturb the page’s existing content or arcade mount.

The staged desktop Level 1 speaker state exposed an animation-related scale collapse: the layout wrapper had returned to unit transform and the displayed art was only at its inner scale. The scene layer now applies an independent `scale: 1.86` to the speaker tower. Direct runtime inspection confirmed each tower renders at approximately **145 × 402 px** with computed scale **1.86**, while the same art had previously rendered around **78 × 224 px**. A deterministic selector assertion now protects that persistent foreground-scale layer.

The live Level 2 item preview was also inspected directly. Every active field object—including the dubplate, bottle, apple core, siren, pill, phone, Lion of Judah, CDJ, mixer, turntable, and 45 adapter—resolved to its approved `/manus-storage/` urban art asset. All preview nodes had transparent backgrounds, zero visible borders, and no box shadow, confirming that the field path does not use generic tiles or white/shape containers. Separate deck-floor reward classes use the same approved mixer and turntable art rather than CSS-built equipment shapes.

The full deterministic negative path was re-run. The curbside comedown mounted with the `selectah-urban-brawler-visual-target` scene at `cover` size and a **204 × 204 px** foreground DJ treatment. After its timer, the game-over overlay mounted with the detailed `5d-selector-rave-stage` scene at `cover` size; its foreground game-over box measured **440 px** wide in a **776 px** viewport and retained the complete **SESSION TERMINATED** hierarchy plus **PLAY AGAIN / RESET** as the first action. This confirms that neither the background nor the foreground hierarchy is collapsing into the formerly shrunken/empty end state.

Rendered review captures confirm that the revised **Too High to Play** and **Cops Seized Your Mixer** scenes retain their detailed urban illustration, a vivid magenta cracked-screen frame, visible scanlines, and the intended focal characters/props. The complete support line remains present in the DOM for each held scene; the capture crops are intentionally framed around the arcade cabinet rather than a text-only review card.

The separate **Wrong Tune My Selectah** and **Thrown Tune** preview captures confirm that the phone-driven and bottle/core-driven crowd outcomes now render independently. Both preserve the club/empty-dancefloor scene, while Wrong Tune displays the requested Wonderwall line and Thrown Tune displays the requested bottle-to-the-face line with its bottle emoji. Each uses the shared magenta cracked-screen and scanline treatment.

Phone-width captures at **375 × 812 px** were reviewed for all four updated hazard scenes. The pill, police, Wrong Tune, and Thrown Tune states each retained the magenta frame, scanlines, illustrated focal art, and non-obscured copy hierarchy inside the cabinet. The compact phone viewport crops the lower continuation of the cabinet, but no overlay content collides with the scene frame or active artwork.

The first standalone 375 × 812 px renderer captures confirmed the mobile title, cabinet frame, pill art, police art, and scene framing were intact. They also showed that the public section heading consumes too much vertical space to prove the complete supporting line within a single review frame. A development-only cabinet-focused capture mode is therefore required before the mobile copy-reading task can be closed.

Cabinet-focused **375 × 812 px** captures then confirmed all four revised hazard outcomes in one frame. **Too High to Play** and **Cops Seized Your Mixer** retain their complete approved messages. The crowd warning was corrected after the first focused capture revealed its inherited negative offset: **Wrong Tune My Selectah** now keeps all three title lines and the Wonderwall line within the frame, while **Thrown Tune** keeps both title lines and the bottle-to-the-face message visible. No functional player layout is affected because the focus wrapper is development-only.
